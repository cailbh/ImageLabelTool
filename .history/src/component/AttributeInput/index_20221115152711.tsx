...
export default class AttributeInput extends React.Component<AttributeInputProps, AttributeInputState> {
    ...
    attrClick = (e,index,type) =>{
        window.onkeyup = null
        window.addEventListener("keyup", (ent) => {
            console.log(ent.key)
            if("Delete" === ent.key){
                let str = e.target.className
                let index = str.split(' ')[2]
                console.log(1111,e,str,index)
                var temp = this.state.attributes
                temp.splice(index, 1)
                this.state = {
                    name: this.state.name,
                            ...
                            return (
                                <div className="attrInput" >
                                    <div className="attrKey">
                                        <input type="text" className={"keyInput attr" + index}  placeholder={item.attrKey}  onClick={(e) => this.attrClick(e, index, "attrKey")} onChange={(e) => this.attrChange(e, index, "attrKey")} />
                                    </div>
                                    <div className='attrValue'>
