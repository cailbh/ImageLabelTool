...
export default function LabelDisplay() {
    ...
    const entityClick = (ent) => {
        ...
        let idx=-1
        let entThis = annotate.current.Arrays.imageAnnotateShower.find(function (item, index, arr) {
            if(item.id == entityId)
            {
                idx = index
            }
    ...
    const drawGraph = ()=>{
            ...
            let Rscale = d3.scaleLinear().range([50,svgWidth]).domain([0,annotate.current.iHeight*annotate.current.iWidth])
            ...
            if (layoutType == 0 || layoutType == 2) {
                ...
                if (layoutType == 0) {
                    ...
                    while(jage){
                        jage =0
                        for (let i = 0; i < entityContext.array.length-1; i++) {
                            const elementI = entityContext.array[i];  
                            let maskI = elementI.rectMask
                            let rSizeI = Rscale(maskI.width*maskI.height)       
                            if (elementI.visAble) {
                                for (let j = i+1; j < entityContext.array.length; j++) {
                                          
                                    const elementJ = entityContext.array[j]
                            if (elementJ.visAble) {
                                    let maskJ = elementJ.rectMask
                                    let rSizeJ = Rscale(maskJ.width*maskJ.height)
                                    let distance = Math.sqrt(Math.pow(elementI.x - elementJ.x, 2) + Math.pow(elementI.y - elementJ.y, 2));
                                    let panSize = ((rSizeI+rSizeJ) - distance)
                                    let tha = Math.atan((elementI.y - elementJ.y)/(elementI.x - elementJ.x))
                                    if(panSize > 0){
                                        jage = 1
                                        let dy = panSize*Math.sin(tha)
                                        let dx = panSize*Math.cos(tha)
                                        if(elementI.y>elementJ.y){
                                            elementJ.y+=dy
                                            elementI.y-=dy
