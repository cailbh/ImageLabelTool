...
export default function LabelDisplay() {
    ...
    const entityRecognizeApi = async()=>{
        ...
        for (var i =0;i<boxs.length;i++){
            ...
            let igd =  _nodes.bCtx.getImageData(boxs[i][0],boxs[i][1],boxs[i][2]-boxs[i][0],boxs[i][3]-boxs[i][1])
            CreateNewResultList(boxs[i][2],boxs[i][3] , "recognize", [boxs[i][0],boxs[i][1]],value,igd);
    ...
    const selectImage = (index) => {
        ...
        task.current.discribe = "《听琴图》是传为北宋宋徽宗赵佶创作的一幅绢本设色工笔画，此画现藏于北京故宫博物院。此图绘松下抚琴赏曲的情景。画面正中一枝苍松，枝叶郁茂，凌霄花攀援而上，树旁翠竹数竿。松下抚琴人着道袍，轻拢慢捻，另二人坐于下首恭听，一侧身一仰面，神态恭谨。画上仅用松竹石表示庭院环境，悠扬的琴韵似在松竹间流动，构图凝练平衡。人物神态刻画细致传神，较好地体现出其心理状态。"
    ...
    const DrawSavedAnnotateInfoToShow = (resultIndex = 0) => {
        ...
        _arrays.imageAnnotateShower.forEach((item, index) => {
            ...
            item.children.forEach((Item, Index) => {
                let ent = _arrays.imageAnnotateShower.find(function (d) { return d.id == Item })
                //绘制子标签
                let name = ent.name
            ...
            if (item.contentType === "polygon") {
                ...
                DrawRect(_nodes.ctx, item.rectMask.xMin, item.rectMask.yMin, item.rectMask.width, item.rectMask.height, item.color,1.5, item.rgb);
            }
            if (_arrays.resultIndex !== 0 && _arrays.resultIndex - 1 === index) {
        ...
        _arrays.relationshipAnnotateShower.forEach((item, index) => {
            ...
            if (_relT == undefined) {
                ...
                // for(let i = 0;i<)
                if ((item.type != "root") && (item.type != "ParentChild")) {
                    let source = _arrays.imageAnnotateShower.find(function (i) { return i.id == sourceId }).rectMask
                    let target = _arrays.imageAnnotateShower.find(function (i) { return i.id == targetId }).rectMask

                    drawArrow(_nodes.ctx, source.xMin, source.yMin, target.xMin, target.yMin, 10, 10, 1, item.color);
                }
            }
        });
    ...
    const DrawSavedAnnotateInfoToMemory = (isRender) => {
        ...
        if (isRender) {
            ...
            _arrays.relationshipAnnotateShower.forEach((item, index) => {
                ...
                // }
                if ((item.type != "root") && (item.type != "ParentChild")) {
                    let source = _arrays.imageAnnotateMemory.find(function (i) { return i.id == sourceId }).rectMask
                    let target = _arrays.imageAnnotateMemory.find(function (i) { return i.id == targetId }).rectMask
    ...
    const MouseUpRemoveDrawRect = () => {
        ...
        var _nodes = annotate.current.Nodes
        var minP = [9999, 9999]
        var maxP = [-9999, -9999]
        ...
        if (maxP[0] - minP[0] >= 5 || maxP[1] - maxP[1] >= 5) {  // 判断矩形绘制距离大于五才认定为有效绘制
            ...
            DrawSavedAnnotateInfoToShow();
            var entImg = _nodes.ctx.getImageData(minP[0],minP[1] , maxP[0]-minP[0], maxP[1]-minP[1])
            console.log(entImg)
            CreateNewResultList(maxP[0], maxP[1], "rect", minP,{},entImg);
            
            DrawSavedAnnotateInfoToShow();
            // _nodes.ctx.putImageData(entImg, 0, 0);
            ReplaceAnnotateMemory();
            let index = annotate.current.Arrays.resultIndex - 1;
            RecordOperation('add', '绘制矩形框', index, JSON.stringify(annotate.current.Arrays.imageAnnotateMemory[index]));
        }
            ...
            for(let i=0;i<inpolList.current.length;i++){
                ...
                but.addEventListener('click',(e)=>{
                    ...
                    if ((targetIndex!=-1)&&(targetIndex != parseInt(annotate.current.sourceIndex) )) {  // 判断绘制距离大于五才认定为有效绘制
                        ...
                        // 保存绘图数据
                        CreateNewRelationshipResultList(sourceId, targetId, "relationship");
                        DrawSavedAnnotateInfoToShow();
            ...
            for(let i=0;i<inpolList.current.length;i++){
                ...
                but.addEventListener('click',(e)=>{
                    ...
                    if ((targetIndex!=-1)&&(targetIndex != parseInt(annotate.current.sourceIndex) )) {  // 判断绘制距离大于五才认定为有效绘制
                        ...
                        ent.showLabel=false
                        // 保存绘图数据
                        CreateNewRelationshipResultList(sourceId, targetId, "ParentChild");
                        _arrays.resultIndex  = 0
    ...
    //----创建新的标定结果标签
    const CreateNewResultList = (lx, ly, contentType, p = [],data = {},igData = {data:[]}) => {
        let _nodes = annotate.current.Nodes;
        let _arrays = annotate.current.Arrays;
        let id = uuid()
        // 将获取到的图像数据去除A通道
        var iData = igData.data
        var imgArr = [0,0,0];
        var igLen = 0
        var rgbCurrent = 0
        ...
        for(var i=0; i<iData.length; i += 4){
            rgbCurrent++
            if((rgbCurrent>=((iDataLen/4 - midL)/2))||(rgbCurrent<=((iDataLen/4 + midL)/2))){
                imgArr.push(iData[i], iData[i+1], iData[i+2])
                imgArr[0]+=iData[i]
                imgArr[1]+=iData[i+1]
                imgArr[2]+=iData[i+2]
        ...
        let color = coloring()
        // let color = "rgb("+ imgArr[0]/igLen+","+imgArr[1]/igLen+","+imgArr[2]/igLen+")"
        let ent = new Entity("")
        ...
        entityContext.array.push(ent)
        updateEntityArray(entityContext.array)
        CreateNewRelationshipResultList(id, "root", "root");
    };
    ...
    //实体点击
    const entityClickOld = (ent) => {
        let entityId = ent.target.id
        attrInddex.current = -1;
        let idx=-1
        let entThis = null
        annotate.current.Arrays.imageAnnotateShower.find(function (item, index, arr) {
            if(item.id == entityId)
            {
                idx = index
                entThis = item
        ...
        annotate.current.Arrays.resultIndex = idx +1
        console.log(entThis)
        nameTp.current = entThis.name
        attributesTp.current = entThis.attribute 
        let vis = true;
        if (layoutType == 0) {
            if(annotate.current.Features.relationshipOn) {
                if(annotate.current.sourceID == ""){
                    annotate.current.sourceID = entityId
                    console.log("s",entityId)
                }
                else{   
                    console.log("t",entityId,entityContext)
                    CreateNewRelationshipResultList(annotate.current.sourceID, entityId, "relationship");
                    annotate.current.sourceID = ""
                }
            }
    ...
    const entityClick = (ent)=>{
        ...
        focusEntId.current = id
        attrInddex.current = -1;
        // console.log(item,children)
        let op = 1 - clickState.current
        clickState.current = op
        // console.log(op)
        for(let i =0;i<children.length;i++){
            let childId = children[i]
            d3.select("#"+childId).attr("opacity",op)
            d3.select("#textName_" + childId).attr("opacity",op)
            // console.log(d3.select("#s-" + childId + "-t-" + id))
            d3.select("#s-" + childId + "-t-" + id).attr("opacity",op)
            d3.selectAll(".s-" + childId + "-t-" + id).attr("stroke-opacity",op)
            d3.select("#sDM-" + childId + "-tDM-" + id).attr("opacity",op)
        }
    }
    //实体鼠标漂浮
    const entityMouseOver = (ent) => {
        if(dragState.current == 0){
            var item = ent.target
            let cx = item.cx.animVal.value
            let cy = item.cy.animVal.value
            focusEntId.current = item.id
            attrInddex.current = -1;
            let element = entityContext.array.find(function (_item, index, arr) {
                return _item.id === item.id
            })
            d3.selectAll(".attr").remove()
            let rSize = 40 - element.level
            d3.selectAll(".entCircle")
                .attr("stroke","#000")
                .attr("stroke-width", "1px")
            d3.select("#"+element.id)
                .attr("stroke",element.color)
                .attr("stroke-width", rSize*0.2)
            var attrSvg = d3.select("#attrG")
                var smallR = 10
                var k = 0
                ...
                var attrStepR = Math.PI *2 / attrLen
                    while (k < attrLen) {
                        ...
                        var arcPath = d3.arc()
                            .innerRadius(rSize+10)
                            .outerRadius(rSize+15);
    ...
    //绘制图谱
    // const drawGraphOld = ()=>{
    //         let divWidth = chart.current.clientWidth*2
    //         let divHeight = chart.current.clientHeight*2
    //         let svgWidth = divWidth
    //         let svgHeight = divHeight
    //         // let svgWidth = annotate.current.iWidth
    //         // let svgHeight = annotate.current.iHeight
    //         // if (svgHeight> svgWidth){
    //         //     svgWidth = svgWidth*(divHeight/svgHeight)
    //         //     svgHeight = divHeight
    //         // }
    //         // else{
    //         //     svgHeight = svgHeight*(divWidth/svgWidth)
    //         //     svgWidth = divWidth
    //         // }
    //         let rSize = 60
    ...
    //         let Rscale = d3.scaleLinear().range([svgWidth/10,svgWidth]).domain([0,annotate.current.iHeight*annotate.current.iWidth])
                
    //         if (layoutType == 2 || layoutType == 1) {
    //             let maxS = -1
    //             let preData = []
    //             if (layoutType == 1) {
    //                 var jage = 1
    ...
    //                     if(element.rectMask.width*element.rectMask.height > maxS) maxS = element.rectMask.width*element.rectMask.height
    //                 } 
    //                     preData.push(element)
                        
    //                 }
    //             // Rscale = d3.scaleLinear().range([50,100]).domain([0,maxS])
    //             // var comX = function(objA,objB){
    //             //     let maskA = objA;
    //             //     let maskB = objB;
    //             //     return ((maskA.x)>(maskB.x))?(1):(0);
    //             //     // return ((maskA.x+maskA.y)>(maskB.y+maskB.x))?(1):(0);
    //             // }; 
    //             // var comY = function(objA,objB){
    //             //     let maskA = objA;
    //             //     let maskB = objB;
    //             //     return ((maskA.y)>(maskB.y))?(1):(0);
    //             // };
    //             // preData.sort(comX);
    //             // for (let i = 0; i < preData.length-1; i++) {
    //             //     let xI = preData[i].x;
    //             //     let yI = preData[i].y;
    //             //     let maskI = preData[i].rectMask;
    //             //     let rSizeI = Rscale(maskI.width*maskI.height)*2;
    //             //     for (let j = i; j < preData.length; j++) {
    //             //         let xJ = preData[j].x;
    //             //         let yJ = preData[j].y;
    //             //         let maskJ = preData[j].rectMask;
    //             //         let rSizeJ = Rscale(maskJ.width*maskJ.height)*2;
    //             //         let distance = Math.sqrt(Math.pow(xJ - xI, 2) + Math.pow(yJ - yI, 2));
    //             //         let disX = xJ - xI;
    //             //         let disY = yJ - yI;
    //             //         if(disX<(rSizeI+rSizeJ)){
    //             //             // if(disX>disY){
    //             //                 if(distance<(rSizeI+rSizeJ)){
    //             //                 preData[j].x = xJ +(rSizeI+rSizeJ)/2  
    //             //                 }  
    //             //             // }
    //             //             // else{
    //             //                 // preData[j].y = yJ +(rSizeI+rSizeJ)/2
    //             //             // }
    //             //         }
    //             //     }
    //             // }
    ...
    //             console.log(preData);
    //             // updateEntityArray(preData)
    //         }
    //             for (let i = 0; i < preData.length; i++) {
    //                 const element = preData[i];
    //                 if (element.visAble) {
    //                     // if (layoutType == 0) {
    //                     //     element.x = (svgWidth*2) * (element.lx - element.ix) / (element.iScale * element.iWidth)
    //                     //     element.y = (svgHeight*2) * (element.ly - element.iy) / (element.iScale * element.iHeight)
    //                     // }
                        
    //                     let mask = element.rectMask
    //                     rSize = Rscale(mask.width*mask.height)
    ...
    //                         .attr("x", element.x)
    //                         .attr("y",  element.y)
    //                         .attr('width',rSize*2*Math.sqrt(3))
    //                         .attr('height',rSize)
    //                         .attr("transform",function(d){
    //                              let bbox = this.getBBox()
    //                             return "translate("+(-bbox.width/2)+","+(-(rSize/2))+")"
    //                         })      
    ...
    //                     inputKey.style.fontSize = rSize+"px"
    ...
    //                     input.style.fontSize = rSize/2+"px"
    //                     if(attrInddex.current==-1||(focusEntId.current != element.id) ){
    ...
    //                     let unitVec = getUnitVector(startA, endA)
    //                     let maskS = eSource.rectMask
    //                     let rSizeS = Rscale(maskS.width*maskS.height)
    //                     let maskE = eTarget.rectMask
    //                     let rSizeE = Rscale(maskE.width*maskE.height)
    //                     startA[0] += rSizeS * unitVec[0]
    //                     startA[1] += rSizeS * unitVec[1]
    //                     endA[0] -= rSizeE * unitVec[0]
    //                     endA[1] -= rSizeE * unitVec[1]
    //                     // let middleA = computeControlPoint(startA, endA);
    //                     let middleA = [(endA[0] + startA[0])/2,(endA[1] + startA[1])/2]
    //                     let lineWidth = 2;
    //                     let path = d3.path()
    //                     path.moveTo(startA[0], startA[1])
    //                     path.quadraticCurveTo(middleA[0], middleA[1], endA[0], endA[1]);
    ...
    //                         .attr("transform",function(d){
    //                              let bbox = this.getBBox()
    //                             return "translate("+(-bbox.width)+","+(-(bbox.height))+")"
    //                         })      
    ...
    //             })
    //         }
    //         else if (layoutType == 0 && (entityContext.array.length > 0)) {
    //             // let nodes = []
    //             // let edges = []
    ...
    const drawGraph = ()=>{
        ...
        let Rscale = d3.scaleLinear().range([5,10]).domain([0,annotate.current.iHeight*annotate.current.iWidth])
            
        if (layoutType == 0 && (entityContext.array.length > 0)) {
            ...
            var forceSimulation = d3.forceSimulation()
                .force("link", d3.forceLink().id((d) => { return d.id }).distance(200))
                .force("charge", d3.forceManyBody().strength(-50))
                .force("center", d3.forceCenter(svgWidth / 2, svgHeight / 2))
                .force("collide",d3.forceCollide().radius(rSize*2).iterations(2));
            forceSimulation.nodes(nodes)
                .on("tick");
            ...
            // //////////////////
            let nodeData = nodes
            let eData = relationshipContext.array
            for (let i = 0; i < nodeData.length; i++) {
                const element = nodeData[i];
                // if (element.visAble) {
                    let color = element.color
                    rSize = 40 -element.level*10
                    var circle = svg.append("circle")
                        // .enter(nodeData)
                        .attr("id", element.id)
                        .attr("class", "entCircle")
                        .attr("cx", element.x)
                        .attr("cy", element.y)
                        .attr("r", rSize)
                        // .attr("fill", "url(#pic" + i + ")")
                    ...
                    var text = svg.append("text")
                        .attr("id", "textName_" + element.id)
                        .attr("class", "textName_" + element.id)
                        .attr("x", element.x)
                        .attr("y",  element.y)
                        // .attr("fill",color)
                        .attr("fill","#000")
                        ...
                        .attr("transform",function(d){
                                let bbox = this.getBBox()
                            return "translate("+(-bbox.width/2)+","+(+(bbox.height/3))+")"
                        })   
                // }
            ...
            for (let i = 0; i < eData.length; i++) {
                let relItem = eData[i]
                if (relItem.type != 'root') {
                    let eSource = nodeData.find(function (item, index, arr) {
                        return item.id === relItem.sourceId
                    })
                    let eTarget = nodeData.find(function (item, index, arr) {
                        return item.id === relItem.targetId
                    })
                    if(eSource.visAble&&eTarget.visAble){
                        let startA = [eSource.x, eSource.y]
                        ...
                        let unitVec = getUnitVector(startA, endA)
                        let maskS = eSource.rectMask
                        let rSizeS = 40 -eSource.level*10
                        let maskE = eTarget.rectMask
                        let rSizeE = 40 -eTarget.level*10
                        startA[0] += rSizeS * unitVec[0]
                        startA[1] += rSizeS * unitVec[1]
                        endA[0] -= rSizeE * unitVec[0]
                        endA[1] -= rSizeE * unitVec[1]
                        ...
                        relSvg.append('path')
                            .attr("id", "s-" + eSource.id + "-t-" + eTarget.id)
                            .attr("class", "s-" + eSource.id + "-t-" + eTarget.id)
                            .attr('d', path.toString())
                            .attr('fill-opacity', '0')
                            .style('stroke', relItem.color)
                        ...
                        var relText = relSvg.append("text")
                            ...
                            .attr('transform',
                                function (t, k) {
                                    return ` rotate(${rotateT(startA, endA)},${midP(startA, middleA, endA)}) translate(${midP(startA, middleA, endA)})`
                                })
                            // .attr("transform",function(d){
                            //     let bbox = this.getBBox()
                            //     return "translate("+(-bbox.width)+","+(-(bbox.height))+")"
                        ...
                        // relSvg.append('path')
                        //     .attr('fill', relItem.color)
                        //     .attr("id", "sD-" + eSource.id + "-tD-" + eTarget.id)
                        //     .attr("class", "sD-" + eSource.id + "-tD-" + eTarget.id)
                        //     .style("display", "none")
                        //     .attr('d', 'M 11 0 C 7 5 -1 9 -10 10 L -10 10 C -5 7 -1 5 1 0 C -1 -5 -4 -9 -10 -10 L -10 -10 C -1 -9 7 -5 11 0 Z')
                        //     .append('animateMotion')
                        ...
                        relSvg.append('path')
                            .attr("id", "sDM-" + eSource.id + "-tDM-" + eTarget.id)
                            .attr('fill', relItem.color)
                            .attr("class", "sDM-" + eSource.id + "-tDM-" + eTarget.id)
                            // .style("display", "none")
                            .attr('d', 'M 0 -10 L 10 0 L 10 0 L 0 10 L 3 0 L 3 0 L 0 -10  ')
                            .attr('transform',
                                function (t, k) {
                                    return ` rotate(${rotate(startA, endA)},${midP(startA, middleA, endA)}) translate(${midP(startA, middleA, endA)})`
                                })
            ...
            forceSimulation.on("tick", (d) => {
                ...
                for (let i = 0; i < nData.length; i++) {
                    ...
                    // if (element.visAble) {
                    // else{
                    //     d3.selectAll(".attr_"+element.id).remove()
                    // }
                        // //////////////////////////

                        var circle = d3.select("#"+element.id)
                            .attr("cx", element.x)
                            .attr("cy", element.y) 

                        var text = d3.select("#textName_"+element.id)
                            .attr("x", element.x)
                            .attr("y", element.y) 
                    // }
                ...
                for (let i = 0; i < eData.length; i++) {
                    let relItem = eData[i]
                    if (relItem.type != 'root') {
                        let eSource = nData.find(function (item, index, arr) {
                            return item.id === relItem.sourceId
                        })
                        let eTarget = nData.find(function (item, index, arr) {
                            return item.id === relItem.targetId
                        })
                        if(eSource.visAble&&eTarget.visAble){
                            let startA = [eSource.x, eSource.y];
                            ...
                            d3.select("#s-" + eSource.id + "-t-" + eTarget.id)
                                .attr("class", "s-" + eSource.id + "-t-" + eTarget.id)
                                .attr('d', path.toString())
    
                            
                            d3.select("#relText_" + relItem.id)
                                .attr('transform',
                                function (t, k) {
                                    let bbox = this.getBBox()
                                    return ` rotate(${rotateT(startA, endA)},${midP(startA, middleA, endA)}) translate(${midTP(bbox,startA, middleA, endA)})`
                                })

                            d3.select("#sDM-" + eSource.id + "-tDM-" + eTarget.id)
                            .attr('transform',
                                function (t, k) {
                                    return ` rotate(${rotate(startA, endA)},${midP(startA, middleA, endA)}) translate(${midP(startA, middleA, endA)})`
                                })
    ...
    useEffect(() => {
        ...
        const attributeInputDiv = document.getElementById('attributeInputDiv')
        attributeInputDiv.onmousedown = function(e){
            document.onmousemove = function(e){
                ...
                if(x<0){
                    x = 0
                }else if(x>canvasContent.offsetWidth-attributeInputDiv.offsetWidth){//判断是否超出外层界限
                    x = canvasContent.offsetWidth-attributeInputDiv.offsetWidth
                }
                
                if(y<0){
                    y = 0
                }else if(y>canvasContent.offsetHeight - attributeInputDiv.offsetHeight){
                    y = canvasContent.offsetHeight - attributeInputDiv.offsetHeight
                }
                
                attributeInputDiv.style.left = x + 'px'
                attributeInputDiv.style.top = y + 'px'
            }
            document.onmouseup = function(e){
                document.onmousemove = null//重新
                ...
                <div className="canvasMain">
                    ...
                    <div id="sideBar" className="toolFeatures">
                        ...
                        <div className="baseInformation">
                            ...
                            <div className="info infoAuthor">作  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;者：<a>{task.current.author}</a></div>
                            <div className="info infoTime">创作时间：<a>{task.current.time}</a></div>
                            <div className="info infoTexture">材&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;质：<a>{task.current.texture}</a></div>
                            
                            <div className="info infoSize">尺&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;寸：<div className="infoDiv sizeInfoDiv">{task.current.size}</div></div>
                            <div className="info infoDescribe">简&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;述：<div className="infoDiv">{task.current.discribe}</div></div>
                        ...
                        <div id="attrDiv">
                            <div className="toolsHead">
                                ...
                                {"AI-Assisted Annotation"}</div>
                            </div>
                            <div className="AITools">
                            <div className="objectDetection AITool">目标检测：
                                <select className="AIInterfaceSelect" onChange={objectDetectionChange.bind(this)}>
                                    <option value={"Faster-rnn"} key={0}>
                                        {"Faster-rnn"}
                                    </option>
                            ...
                            <div className="relationshipPrediction AITool">关系预测：
                                <select className="AIInterfaceSelect" onChange={objectDetectionChange.bind(this)}>
                                    <option value={"Graph RCNN"} key={0}>
                                        {"Graph RCNN"}
                                    </option>
                            ...
                            <div className="modelOptimization AITool">模型优化：
                                <select className="AIInterfaceSelect" onChange={objectDetectionChange.bind(this)}>
                                    <option value={"Active Learing"} key={0}>
                                        {"Active Learing"}
                                    </option>
                            ...
                            <div className="AISelect AITool">图片检索：
                            <input className="AIInterfaceInput"></input>
                            </div>
                            </div>
                                <p className="featureList labelShower focus" title="标注结果显示开关">
                        ...
                        <div className="scaleList">
                            ...
                            <div className="scaleListHead">
                                <div className="scaleListLogo">Thumbnail List</div></div> 
                    ...
                    <div id="imagDiv" className="imagDiv">
                        ...
                        <div className="labelHead">
                            ...
                            <div className="labelToolLogo">Annotation Tools</div>
                ...
                <Button type="primary" onClick={() => changeToolType(2)}>实体属性</Button> */}
                    ...
                    <div id="graphDiv" className="graphDiv">
                        <div className="graphHead">
                            <div className="graphLogo">Graph Interactions</div>
