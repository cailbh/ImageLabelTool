...
export default function LabelDisplay() {
    ...
    useEffect(() => {
        ...
        attributeInputDiv.onmousedown = function(e){
            var dx = e.offsetX//和下面的两种方法皆可，offsetX获取鼠标位置在父元素中的坐标(事件属性)
            var dy = e.clientY - attributeInputDiv.offsetTop//client获取返回当事件被触发时鼠标指针向对于浏览器页面（或客户区）的水平坐标，offsetTop(dom元素方法)返回元素相对于最近定位父级元素的坐标，若在所有的父级上都没有定位，则相对于整个文档，dx和dy主要是获取事件发生时鼠标相对于父元素的位置
            document.onmousemove = function(e){
                var x = e.clientX - dx//x和y返回移动之后的坐标
                var y = e.clientY - dy
